...
const createProblem = async (req, res) => {
    ...
    try {
        const message = await validateReferenceSolutions(referenceSolution, visibleTestCases, hiddenTestCases);
        if (message) {
...
const updateProblem = async (req, res) => {
    ...
    try {
        ...
        const newUpdatedProblem = await Problem.findByIdAndUpdate(
            id,
            { title, description, difficulty, tags, visibleTestCases, hiddenTestCases, boilerplateCode, problemCreator, referenceSolution },
            { new: true }
        );
        ...
        return res.status(200).json({ message: "Problem Updated Successfully", newUpdatedProblem });
    } catch (error) {
        console.log(error);
...
const individualProblemSubmissions = async (req, res) => {
    try {
        ...
        const submissionss = await Submission.find({ userId, problemId }).explain('executionStats');
        return res.status(200).json({ message: "Submission fetched successfully", submissionss, totalSubmissions: submissions.length, submissions });
    } catch (error) {
        console.log(error);
...
export { createProblem, updateProblem, deleteProblem, getAllProblems, getProblemById, individualSolvedProblems, individualProblemSubmissions };